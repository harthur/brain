...
/*
  ...
  */
NeuralNetwork.prototype.createTrainStream = function(opts) {
  opts = opts || {};
...
function TrainStream(opts) {
  ...
  var self = this;
  self.neuralNetwork = opts.neuralNetwork;
  self.dataFormatDetermined = false;

  self.inputKeys = [];
  self.outputKeys = []; // keeps track of keys seen
  self.i = 0; // keep track of the for loop i variable that we got rid of
  self.iterations = opts.iterations || 20000;
  self.errorThresh = opts.errorThresh || 0.005;
  self.log = opts.log || false;
  self.logPeriod = opts.logPeriod || 10;
  self.callback = opts.callback;
  self.callbackPeriod = opts.callbackPeriod || 10;
  self.floodCallback = opts.floodCallback;
  self.doneTrainingCallback = opts.doneTrainingCallback;
  // Should probably throw an error if opts.floodCallback or opts.doneTrainingCallback are undefined
  ...
  self.on('finish', self.finishStreamIteration);

  return self;
}

util.inherits(TrainStream, Writable);
...
TrainStream.prototype.finishStreamIteration = function() {
  ...
  if (this.log && (this.i % this.logPeriod == 0)) {
    console.log("iterations:", this.i, "training error:", this.error);
  }
  ...
  if (this.i < this.iterations && this.error > this.errorThresh) {
    if (typeof this.floodCallback === 'function') {
