...
/*
  This will also create a WriteStream for us to send the training data to.
...
NeuralNetwork.prototype.initTrainStream = function(options) {
  ...
  this.trainStream = {};
  this.trainStream.inputKeys = [];
  this.trainStream.outputKeys = []; // keeps track of keys seen
  this.trainStream.i = 0; // keep track of the for loop i variable that we got rid of
  this.trainStream.iterations = options.iterations || 20000;
  this.trainStream.errorThresh = options.errorThresh || 0.005;
  this.trainStream.log = options.log || false;
  this.trainStream.logPeriod = options.logPeriod || 10;
  this.trainStream.callback = options.callback;
  this.trainStream.callbackPeriod = options.callbackPeriod || 10;
  this.trainStream.floodCallback = options.floodCallback;
  this.trainStream.doneTrainingCallback = options.doneTrainingCallback;
  // Should probably throw an error if options.floodCallback or options.doneTrainingCallback are undefined

  this.trainStream.size = 0;
  this.trainStream.count = 0;

  this.trainStream.error = 1;
  this.trainStream.sum = 0;
}
...
NeuralNetwork.prototype._write = function(chunk, enc, next) {
  ...
  if (!this.dataFormatDetermined) {
    this.trainStream.size++;
    this.trainStream.inputKeys = _.union(this.trainStream.inputKeys, _.keys(chunk.input));
    this.trainStream.outputKeys = _.union(this.trainStream.outputKeys, _.keys(chunk.output));
    this.trainStream.firstDatum = this.trainStream.firstDatum || chunk;
    return next();
  }

  this.trainStream.count++;
...
NeuralNetwork.prototype.trainDatum = function(datum) {
  ...
  this.trainStream.sum += err;
}

NeuralNetwork.prototype.finishStreamIteration = function() {
  if (this.dataFormatDetermined && this.trainStream.size !== this.trainStream.count) {
    console.log("This iteration's data length was different from the first.");
  }

  if (!this.dataFormatDetermined) {
    // create the lookup
    this.inputLookup = lookup.fromArray(this.trainStream.inputKeys);
    this.outputLookup = lookup.fromArray(this.trainStream.outputKeys);

    var data = this.formatData(this.trainStream.firstDatum);
    ...
    if (typeof this.trainStream.floodCallback === 'function') {
      this.trainStream.floodCallback();
    }
    return;
  }

  this.trainStream.error = this.trainStream.sum / this.trainStream.size;

  if (this.trainStream.log && (this.trainStream.i % this.trainStream.logPeriod == 0)) {
    console.log("iterations:", this.trainStream.i, "training error:", this.trainStream.error);
  }
  if (this.trainStream.callback && (this.trainStream.i % this.trainStream.callbackPeriod == 0)) {
    this.trainStream.callback({
      error: this.trainStream.error,
      iterations: this.trainStream.i
    });
  }
  ...
  if (this.trainStream.i < this.trainStream.iterations && this.trainStream.error > this.trainStream.errorThresh) {
    if (typeof this.trainStream.floodCallback === 'function') {
      return this.trainStream.floodCallback();
    }
  } else {
    // done training
    if (typeof this.trainStream.doneTrainingCallback === 'function') {
      return this.trainStream.doneTrainingCallback({
        error: this.trainStream.error,
        iterations: this.trainStream.i
      });
    }
